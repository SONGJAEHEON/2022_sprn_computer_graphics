...
def exp(rv):
    ...
    cosine = np.cos(ang)
    sine = np.sin(ang)
    R = np.array([[cosine+x*x*(1-cosine), x*y*(1-cosine)-z*sine, x*z*(1-cosine)+y*sine],
...
def log(R):
    ...
    return theta*(normalized(np.array([v1, v2, v3])))
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2017029834-9-2', None,None)
    if not window:
        glfw.terminate()
        return
